...
var VKModule = {
  ...
  getPluginPath: function (url, getAddOnVersion, callback) {
    chrome.tabs.sendMessage(currentTabId, { action: 'getAudioSrc' }, function (response) {
      if (response) {
        callback(response.audioSrc);
      }
    });
  }
};
...
var Mp4UploadModule = {
  ...
  getPluginPath: function (url, getAddOnVersion, callback) {
    ...
    // get embedded data
    $.ajax({
      url: 'http://www.mp4upload.com/embed-' + id + '.html', success: function (data) {
